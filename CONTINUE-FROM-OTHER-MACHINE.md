...
31. ~~qm TypeScript adapter~~ — AUDITED (session 13). The qm bridge is **already wired** via the Python CLI tool (`deploy/qm-pacgate/sandbox/tools/pacgate-qm/pacgate_qm.py`), not the TS adapter. The TS adapter (`pacgate-adapters/typescript/`) is a contract library with 8 passing unit tests — it is NOT used by the qm sandbox (qm sandbox tools are CLI binaries, not TS modules). The Python CLI is the live wiring: `qm check` passes, `qm sandbox build` passes, and full e2e tested against `ghcr.io/jzkk720/pacgate-api:0.1.0` (register → login → ensure-matter → memory save/get → workflow search — all pass). The TS adapter remains available as a library for non-qm consumers or a future migration target if qm gains a TS plugin surface.
