...
export const enUS: Translations = {
  ...
  // PacGate report templates
  pacgate: {
    title: "PacGate Legal Templates",
    description: "Prefill a PacGate-boundary-safe report prompt.",
    templates: [
      {
        label: "Patent Disclosure (交底书)",
        prompt:
      ...
      {
        label: "Counterparty Risk Report",
        prompt:
          "你是一名法律合规研究员，负责生成一份《交易对手风险核查报告》。请严格遵循下面的步骤，只使用 PacGate 提供的工具（read_document、kb_search、write_file、present_files）。不要尝试调用 Bash、浏览器或本地脚本。\n\n【核查对象 / Subject】\n交易对手：____________________（公司全称 / 统一社会信用代码）\n\n【第一步：检索既有知识】\n使用 kb_search 检索本所知识库中与“____________________”相关的核查记录、既有报告或风险提示，检索关键词不少于 3 个。\n\n【第二步：核查公开信息】\n1) 查询交易对手的涉诉情况；\n2) 查询行政处罚记录；\n3) 查询被执行人记录。\n若某项查询无结果，明确写明“未查询到”，不要编造。\n\n【第三步：撰写报告】\n基于检索与查询结果，撰写一份结构完整的《交易对手风险核查报告》，包含：\n1. 交易对手基本信息\n2. 涉诉情况\n3. 行政处罚\n4. 被执行人记录\n5. 综合风险评级（高/中/低，并说明依据）\n6. 结论与建议\n\n【第四步：写入文件】\n使用 write_file 将完整报告写入：\n- description：交易对手风险核查报告 — <公司名称>\n- path：/mnt/user-data/outputs/交易对手风险核查报告_<公司名称>.md\n- content：完整报告正文\n\n写入完成后，用 present_files 呈现该文件路径，并做 3-5 句交付摘要。\n\n【约束】\n- 只使用 PacGate 边界内的工具；不要调用 Bash、浏览器或本地脚本。\n- 若关键信息缺失（如交易对手名称），先列出缺失项并给出建议，不要擅自编造。\n- 全文使用规范中文，避免口语化表达。",
      },
    ],
  },
