...
export function useDeleteThread() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // A 404 means the thread is already gone — the desired end state. The
      // prior `apiClient.threads.delete` call hits the same gateway handler
      // (nginx rewrites /api/langgraph/threads/* to /api/threads/*) and removes
      // the thread_meta row, so this second delete's ownership guard 404s.
      // Treat it as success to keep the delete idempotent.
