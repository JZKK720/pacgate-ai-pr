...
function PacgateTemplateList() {
  ...
  const handleTemplateClick = useCallback(
    (prompt: string | undefined) => {
      ...
      setTimeout(() => {
        ...
        if (textarea) {
          const selStart = prompt.indexOf("____");
          const selEnd = selStart === -1 ? -1 : selStart + 4;
          if (selStart !== -1 && selEnd !== -1) {
  ...
  return (
    <div className="flex w-full max-w-full flex-col items-center gap-2 px-4 sm:w-fit sm:px-0">
      <div className="text-muted-foreground text-xs font-medium">
        {t.pacgate.title}
      </div>
      <Suggestions className="min-h-10 w-full max-w-full justify-center px-4 sm:w-fit sm:px-0">
        {t.pacgate.templates.map((template) => (
          <Suggestion
            key={template.label}
