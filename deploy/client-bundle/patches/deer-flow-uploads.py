...
async def list_uploaded_files(thread_id: str, request: Request) -> UploadListResponse:
    ...
    for f in result["files"]:
        ...
        # Populate markdown metadata for a converted sibling (.md alongside
        # an office/pdf file), and original_filename for renamed uploads.
        if f.get("markdown_file") is None:
            stem = Path(f["filename"]).stem
            md_name = stem + ".md"
            md_path = sandbox_uploads / md_name
            if md_path.exists():
                f["markdown_file"] = md_name
                f["markdown_path"] = str(md_path)
                f["markdown_virtual_path"] = upload_virtual_path(md_name)
                f["markdown_artifact_url"] = upload_artifact_url(thread_id, md_name)
        if f.get("original_filename") is None:
