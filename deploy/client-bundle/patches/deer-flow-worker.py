...
async def run_agent(
    ...
    *,
    ...
    try:
        ...
        # Pacgate: inject the conversation-history token estimate into the config
        # context so _make_lead_agent can route local vs cloud based on real prompt
        # complexity (not just the constant tool-schema count). graph_input carries
        # the messages; this is the only place both are in scope before the agent is
        # built.
        _inject_message_token_estimate(config, graph_input)
...
def _estimate_message_tokens(graph_input: dict) -> int | None:
    """Estimate the conversation-history token count from graph_input messages.

    Best-effort: sums the char length of each message's text content and divides
    by ~4 (chars/token). Returns None if no messages are present. This is the
    only per-request signal that actually varies with complexity (tool results,
    long research threads, etc.) and lets auto-routing distinguish simple chats
    from genuinely large/complex runs.
    """
        ...
        if isinstance(content, str):
            ...
            for block in content:
                if isinstance(block, dict):
                    c = block.get("text") or block.get("content")
                    if isinstance(c, str):
                        total_chars += len(c)
    if total_chars == 0:
        return None
...
def _inject_message_token_estimate(config: dict, graph_input: dict) -> None:
    """Inject the conversation-history token estimate into config context.

    The agent's ``_get_runtime_config`` reads ``configurable`` and ``context``.
    We put ``pacgate_message_tokens`` into ``context`` so ``_make_lead_agent``
    can read it via ``cfg.get("pacgate_message_tokens")``.
    """
    tokens = _estimate_message_tokens(graph_input)
    if tokens is None:
        return
    context = config.get("context") or {}
    if isinstance(context, dict):
        context["pacgate_message_tokens"] = tokens
    else:
        # context may be a dict-like object; setdefault a fresh dict
        context = {"pacgate_message_tokens": tokens}
