...
def _resolve_model_name(
    ...
    *,
    ...
    # An explicit request always wins (manual picker / API context).
    if requested_model_name and app_config.get_model_config(requested_model_name):
        return requested_model_name

    # Pacgate auto-routing: only when the caller did NOT pin a model.
    if not requested_model_name:
        routing = getattr(app_config, "model_routing", None)
        # model_routing is an extra="allow" field, so it may be a dict (as
        # loaded from YAML) or a Pydantic model. Read keys from either.
        def _routing_get(key, default=None):
            if routing is None:
                return default
            if isinstance(routing, dict):
                return routing.get(key, default)
            return getattr(routing, key, default)

        if _routing_get("enabled", False):
            local_model = _routing_get("local_model") or default_model_name
            cloud_model = _routing_get("cloud_model")
            ...
            if cloud_model and app_config.get_model_config(cloud_model):
                ...
                over_message_limit = (
                    ...
                    and message_tokens >= message_threshold
                )
                over_tool_limit = (
                    tool_count is not None
                    and tool_threshold is not None
                ...
                return local_model if app_config.get_model_config(local_model) else default_model_name
...
def _make_lead_agent(config: RunnableConfig, *, app_config: AppConfig):
    ...
    # Pacgate auto-routing: count the bound tool surface AND read the
    # conversation-history token estimate injected by the worker patch. The tool
    # surface is constant (~151 tools), so it cannot distinguish simple from
    # complex; the message_tokens (from graph_input) is the variable signal that
    # grows with multi-step research and long tool results.
    try:
        _tool_count = len(get_available_tools())
    except Exception:
        _tool_count = None
    _message_tokens = cfg.get("pacgate_message_tokens")
    if not isinstance(_message_tokens, int) or _message_tokens < 0:
