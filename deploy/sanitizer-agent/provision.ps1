...
$login = Invoke-WebRequest -Uri "$DeerFlowUrl/api/v1/auth/login/local" -Method Post `
    ...
    -TimeoutSec 15 -SessionVariable session
if ($login.StatusCode -ne 200) {
    Write-Error "deer-flow login failed (HTTP $($login.StatusCode))."
}
$cookie = ($session.Cookies.GetCookies($DeerFlowUrl) | ForEach-Object { "$($_.Name)=$($_.Value)" }) -join '; '
...
$existing = Invoke-RestMethod -Uri "$DeerFlowUrl/api/agents" -Headers $headers -TimeoutSec 10 -ErrorAction SilentlyContinue
$hasSanitizer = $false
...
if ($hasSanitizer) {
    Invoke-RestMethod -Uri "$DeerFlowUrl/api/agents/sanitizer" -Method Put -Headers $headers -Body $body -ContentType 'application/json' -TimeoutSec 30 | Out-Null
    Write-Output 'OK: sanitizer agent updated'
} else {
    Invoke-RestMethod -Uri "$DeerFlowUrl/api/agents" -Method Post -Headers $headers -Body $body -ContentType 'application/json' -TimeoutSec 30 | Out-Null
